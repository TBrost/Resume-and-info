...
Lead a small team of consultants and coordinated between the client and my team to ensure consistent communication and management of expectations and project scope throughout the duration of the project. Our task was to design and implement and automated analytics dashboard that would allow the client to better understand and react to their consumers activity and needs when using the software provided by the client. <a href="javascript:showhide('detailsIworq')"
...
Developed a primary script in R which that accessed the clients stored analytics data, cleaned, arranged, summarized and prepared it for analysis before storing it in the clients desired database system. Its primary abilities are as follows:
...
-   Developed secodary script in R that loads and aggregates a customizable range of historical cleaned data from the partitioned clean data stored in Amazon S3 into Power BI.
...
Took primary dashboard built by the rest of the team and overhauled it by adding user interactive selection, searching, and filtering features before converting it into a web hosted report dashboard easily shareable and accessible to members within the clients organization.
:::
:::
:::
...
Managed a small sub-team as part of a larger effort to aggregate and reorganize over 15 separate corporate websites containing thousands of documents, multimedia, and blog posts into a collected whole.
